...
def map_config_findings_to_sh(args):
    ...
    new_findings = []
    (finding_id, account_id, config_rule_name, resource_type, resource_id, region,
     new_status, new_recorded_time, old_recorded_time, config_rule_arn) = args

    compliance_status = get_compliance_and_severity(new_status)
    description = f"config-{account_id}-Base"
    ...
    new_findings.append({
        ...
        "Id": finding_id,
        "ProductArn": f"arn:aws:securityhub:{region}:{audit_account_id}:product/{audit_account_id}/default",
    ...
    if new_findings:
        try:
            ...
            if response['FailedCount'] > 0:
                print(
                    f"Failed to import {response['FailedCount']} findings")
...
def parse_message(event):
    ...
    if event['detail']['messageType'] == 'ComplianceChangeNotification' and "securityhub.amazonaws.com" not in event['detail']['configRuleARN']:
        ...
        finding_id = f"{account_id}-{region}-{config_rule_name}-{resource_name if resource_name is not None else resource_id}"
        args = [
            finding_id,
            account_id,
            config_rule_name,
            resource_type,
            resource_name if resource_name is not None else resource_id,
            region,
            new_status,
            new_recorded_time,
            old_recorded_time,
